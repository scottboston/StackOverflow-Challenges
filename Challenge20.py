...
if __name__ == '__main__':
    ...
    scenarios = {'Scenario one':  r'C:\Users\scott\PycharmProjects\StackOverflow-Challenges\data\challenge_20_s1.txt',
    ...
    for scenario, file in scenarios.items():
        print(f"\n{scenario}:\n")
        df = pd.read_csv(file, sep=': ', engine='python', names=['name', 'guest', 'flavor'])
        df_out = df.groupby('flavor')['guest'].agg(['sum', 'count']).sum(axis=1).mul(1.5).floordiv(-9).mul(-1)
